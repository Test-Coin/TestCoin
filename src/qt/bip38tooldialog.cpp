...
bool isValidPassabetase(QString strPassabetase, QString& strInvalid)
{
    for (int i = 0; i < strPassabetase.size(); i++) {
        if (!validChar.contains(strPassabetase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(strPassabetase[i]))
                continue;

            strInvalid = strPassabetase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qstrPassabetase = ui->passabetaseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassabetase(qstrPassabetase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_ENC->setText(tr("The entered passabetase is invalid. ") + strInvalid + QString(" is not valid") + QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
        return;
    }
    ...
    std::string encryptedKey = BIP38_Encrypt(EncodeDestination(addr), qstrPassabetase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
...
{
    string strPassabetase = ui->passabetaseIn_DEC->text().toStdString();
    string strKey = ui->encryptedKeyIn_DEC->text().toStdString();

    uint256 privKey;
    bool fCompressed;
    if (!BIP38_Decrypt(strPassabetase, strKey, privKey, fCompressed)) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") + tr("Please check the key and passabetase and try again."));
        return;
    }
